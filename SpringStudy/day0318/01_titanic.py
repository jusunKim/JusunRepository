...
# sns.countplot(x='Pclass',data=data,hue='TrainSplit') 골고루 섞여있는지 보기
# plt.show()
...
# sns.catplot(x='Ticket', y='Survived', data=data[data['TrainSplit']=='Train'], kind='bar')
# plt.xticks(rotation=90)
# plt.show()

'''
라벨인코딩: 문자를 숫자로 변경
sklearn.preprocessiong.LabelEncoder
female 0, male 1 이런 식으로 위에서 일일이 바꿧엇음
근데 개수 많아지면 일일이 하기 귀찮으니까 얘를 이용해보자
'''
...
원핫인코딩:
문자열데이터를 숫자형ㅇ으로 변환.
하나의 열을 범주 개수만큼 분할에 범주에 해당하는 열에는 1, 나머지에는 0
pd.get_dummies()
drop_first=True 하면 첫번째 열을 삭제함. (왜 ㄱㅊ나면, 열 하나 없어도 범주 구분할 수 있기 때문. 
범주가 총 3개면 1 0 / 0 1 / 0 0 이렇게하면되니까)
'''

#탑승항구, Cabin, [Ticket 원핫인코딩 하자
for col in ['Embarked','Cabin','Ticket']:
    data[col] = data[col].astype('category') #자료형을 범주형으로 바꾼ㅁ
    data = pd.get_dummies(data, columns=[col], prefix=col[:3], drop_first=True)
    # 전에는 걍 pd.get_dummies(data) 일케 햇었는데 특정 컬럼만 원핫인코딩 하고싶으면 columns=~
print(data.loc[:,['Emb_Q','Emb_S']].head())

'''피처 스케일링
학습에 사용할 피처의 스케일을 0~1로 정규화
klearn.preprocessing.MinMaxScaler'''

from sklearn.preprocessing import MinMaxScaler
scaler = MinMaxScaler()

#숫자가 아닌 건 들어내야댐 - TrainSplit피처를 제외시키자
scaled_cols = [col for col in data.loc[:, "Pclass":].columns if col != "TrainSplit" ]#passengerId, Survived다음열인 Pclass부터
print(scaled_cols)
...
#원본으로부터 스케일링할 대상이 되는 데이터만 일단 갖고옴
...
#스케일링된 데이터를 원본에 설정한다
# data.loc[:,scaled_cols] = data_scaled[:,:] 실패함
df = pd.DataFrame(data_scaled, columns=scaled_cols) #df새로만들엇음
print(df.head())

#여기에 Survived, TrainSplit추가해야댐
df["Survived"] = data.loc[:,"Survived"]
df["TrainSplit"] = data.loc[:,"TrainSplit"]
print(df.head())

#여기 실패해서 쌤꺼 복붙한 02로 넘어감.